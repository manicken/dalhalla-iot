...
namespace DALHAL {
    ...
    namespace JsonSchema {
        ...
        //volatile const void* keep_SchemaHardwarePin = &DALHAL::JsonSchema::SchemaHardwarePin::RegistryDefine;
        ...
        int SchemaHardwarePin::ExtractFrom(const JsonVariant& jsonObj) const {
            if (jsonObj.containsKey(this->name)) {
                return jsonObj[this->name].as<signed int>();
            } else {
                return -1;
            }
        }
        ...
        void SchemaHardwarePin::SchemaToJson(const SchemaTypeBase& fieldSchema, StringBuilderStreamer& sbs, SchemaEmitMode mode) {
            ...
            sbs.write_json_value_separator(); sbs.write_jsonMemberStart(F("mode"));
