                ...
                bool MergeActions(ScriptTokens& _tokens) {
                    ...
                    for (int i = 0; i < tokenCount; ++i) {
                        ...
                        while (j < end) {
                            ...
                            // Count tokens for current logical action line, respecting DALHAL_SCRIPTS_EXPRESSIONS_MULTILINE_KEYWORD continuation
                            while (j + lineTokenCount < end) {
                                // If token line is current line, include it
                                if (tokens[j + lineTokenCount].line == currentLine) {
                                    lineTokenCount++;
                                }
                                // If last token on current line is DALHAL_SCRIPTS_EXPRESSIONS_MULTILINE_KEYWORD, include next line tokens
                                else if (lineTokenCount > 0 && tokens[j + lineTokenCount - 1].type == ScriptTokenType::ActionJoiner) { // .Equals(DALHAL_SCRIPTS_EXPRESSIONS_MULTILINE_KEYWORD)) {
                ...
                bool VerifyActionBlocks(ScriptTokens& _tokens) {
                    ...
                    for (int i = 0; i < tokenCount; ++i) {
                        ...
                        if (false == ExtractAssignmentParts(_tokens)) {
                            token.ReportTokenError(F("Invalid assignment in action block"));
                            anyError = true;
                            continue;
                        }
                        ...
                        if (Expressions::IsExpressionEmpty(AssignmentParts::rhs)) {
                            token.ReportTokenError(F("RHS expression cannot be empty"));
                            anyError = true;
                            continue;
                        }
                        // Validate RHS
                        if (false == Expressions::ValidateExpression(AssignmentParts::rhs)) {
                            token.ReportTokenError(F("RHS expression validation failed"));
                            anyError = true;
                        }
                ...
                bool ExtractAssignmentParts(ScriptTokens& tokens) {
                    ...
                    if (!foundAssignmentOperator) {
                        ...
                        if (match == nullptr) {
                            ...
                            currentStartToken.ReportTokenError(F("!!!!!!!!!!!!!!!!!! (assignment operator)/(func call)  not found"));
                            return false;
                        }
                        ...
                        if (foundAssignmentOperatorToken->ContainsPtr(prevChar) && Expressions::IsSingleOperator(*prevChar)) {
                            ...
                            if (*prevChar == '<' || *prevChar == '>') {
                                ...
                                if (!foundAssignmentOperatorToken->ContainsPtr(prevChar - 1) || *(prevChar - 1) != *prevChar) {
                                    foundAssignmentOperatorToken->ReportTokenError(F("Invalid compound shift assignment (expected <<= or >>=)"));
                                    return false;
                                }
                            }
                        }
                    ...
                    else // currentStartToken != *foundAssignmentOperatorToken
                    {
                        ...
                        if (foundAssignmentOperatorToken->ContainsPtr(foundAssignmentOperator+1)) {
                            if (assignmentTokenIndex != startIndex) {
                                currentStartToken.ReportTokenError(F("!!!!!!!!!!!!!!!!!! ExtractAssignmentParts expected assignmentTokenIndex mismatch"));
                                return false;
                            }
