...
void Actuator::configureISRData(gpio_num_t& somePin, GpioRegType regType) {
    ...
    bool Actuator::VerifyJSON(const JsonVariant &jsonObj) {
        ...
        const char* uidStr = GetAsConstChar(jsonObj, HAL_JSON_KEYNAME_UID); // used for eventual errors
    ...
    void Actuator::loop() {
        ...
        if (isr_data.handled) {
            ...
            if (isr_data.motorOn) {
                isr_data.motorOn = false;
                stopMotor(); // if for some reason the ISR missed it
    ...
    HALOperationResult Actuator::exec_drive_to_min(Device* device) {
        static_cast<Actuator*>(device)->stopMotor(); // direct call no vtable
